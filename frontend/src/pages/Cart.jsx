...
// import CartTotal from '../compnents/CartTotal';
...
//                 <img onClick={() => updateQuantity(item._id, item.color, 0)} className='w-4 mr-4 sm:w-5 cursor-pointer' src={assets.carticon} alt="" />
//               </div>
//             )
...
const Cart = () => {
    ...
    useEffect(() => {
        if (products.length > 0) {
            ...
            console.log("Cart Data:", tempData); // Inspect cartData
            setCartData(tempData);
        }
    }, [cartItems, products]);
        ...
        <div className='border-t pt-14'>
            ...
            <div>
                {cartData.map((item, index) => {
                    const productData = products.find((product) => product._id === item._id);
                    if (!productData) {
                        console.error(`Product with ID ${item._id} not found in products array.`);
